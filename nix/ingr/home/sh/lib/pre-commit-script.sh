...
lisp=() scheme=() rkt=() proto=() docker=() xml=()
...
run_and_add() {
  # run command on files; then git add only those files that still exist
  local tool="$1"; shift
  local -a files=("$@")
  [ "${#files[@]}" -gt 0 ] || return 0
  echo "→ ${tool}: ${#files[@]} file(s)"
  # shellcheck disable=SC2068
  "$tool" ${files[@]}
  git add -- "${files[@]/#/./}" 2>/dev/null || git add -- "${files[@]}"
}
